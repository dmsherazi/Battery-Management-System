...
/* Back fill the plot with existing data */
    for (int i=0; i<plotEnd; i++)
    {
        int index = (i+plotStartIndex) % NUMBER_POINTS;
        points << QPointF(xData[index],source1[index]);
    }
    d_curve1->setSamples(points);
    PowerManagementMonitorUi.qwtPlot1->replot();
}
...
/* Back fill the plot with existing data */
    ...
    {
        int index = (i+plotStartIndex) % NUMBER_POINTS;
        points << QPointF(xData[index],source2[index]);
    }
    d_curve2->setSamples(points);
    PowerManagementMonitorUi.qwtPlot2->replot();
}
...
the selected series. */
    ...
    float current = breakdown[1].simplified().toFloat();
    float voltage = breakdown[2].simplified().toFloat();
...
/* Fill the data arrays */
    ...
    else if (breakdown[0].simplified() == "dM1")
    {
        ...
        {
/* process plots */
/* If the x axis offset is zero and the plot reaches the end of the graph
(VISIBLE_POINTS), then jump back to allow space for the next set of data.
If offset is not zero, the graph will grow to the plot end then remain steady. */
/* "xoffset" is the number of points to step the graph back. */
/* "plotEnd" is the location of the plot end on the graph x scale. */
/* "plotStartIndex" is the index of the first visible data item in the array. */ 
            int plotLength = VISIBLE_POINTS*xSamples;
            if ((plotEnd == plotLength) && (xoffset == 0))
            {
                plotStartIndex = (plotStartIndex+JUMP*xSamples) % NUMBER_POINTS;
                replot(PowerManagementMonitorUi.xoffsetSlider->value());
            }
...
This computes the length of the plot region from the index of the plot
start in the data array, wipes the plot and replots from the old data.

@param int offset: the x-axis offset number to apply.
*/
void PowerManagementMonitorGui::replot(int offset)
{
/* The number of data points defining the range of the plot. */
    int plotLength = VISIBLE_POINTS*xSamples;
/* If the offset was zero, start from normal operation at xindex which
...
this remains the same until a return to realtime. */
    if (xoffset == 0) lastIndex = xindex;
    xoffset = (20-offset)*20;
/* plotStartIndex is the array index where the graphing starts */
...
to bring the curve to the grid line. */
    int stepback = lastIndex % 20 + xoffset;
...
/* plotEndIndex is the time index where the plotting ends. */
    int plotEndIndex = lastIndex - stepback;
    if (plotEndIndex < 0) plotEndIndex = 0;
/* If data has moved on too far from last change, set plot to earliest history */
    if (xindex - lastIndex > NUMBER_POINTS - plotLength)
        plotEndIndex = xindex - NUMBER_POINTS + plotLength;
/* Work back to the start array index. Deal with realtime to avoid extra jumps */
    if (plotEndIndex > plotLength)
    {
        plotStartIndex = (plotEndIndex-plotLength) % NUMBER_POINTS;
        if (xoffset == 0)
            plotStartIndex = (plotEndIndex+20-plotEndIndex%20-plotLength) % NUMBER_POINTS;
    }
/* At the shorter plots allow plotEndIndex to be shorter than the plot size */
    else if (lastIndex > plotLength) plotEndIndex = plotLength;
/* plotEnd is the effective length of the plot in xSamples steps */
    plotEnd = (NUMBER_POINTS + plotEndIndex - plotStartIndex) % NUMBER_POINTS;
    if (plotEnd > plotLength) plotEnd = plotLength;
    if (plotStartIndex+plotLength > lastIndex)
        xRangeMin = (float)plotStartIndex;
    else
        xRangeMin = (float)(plotEndIndex-plotEnd);
    if (xRangeMin < 0) xRangeMin = 0;
...
qDebug() << "plot" << plotEnd << plotStartIndex << plotEndIndex << xindex << lastIndex << stepback;
...
{
    xSamples = value;
/* Refresh the plot by calling xoffset with current value. */
    replot(PowerManagementMonitorUi.xoffsetSlider->value());
}
